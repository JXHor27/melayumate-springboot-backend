package com.example.demo.modules.flashcard.controller;
...
public class FlashcardListController {
    ...
    public ResponseEntity<Lesson> updateLessonAvailability(@PathVariable String listId, @RequestBody Map<String, Boolean> payload) {
        boolean isRandom = payload.get("isRandom");
        listService.updateOrder(listId, isRandom);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
    }

    @PatchMapping("/{listId}/language")
    public ResponseEntity<Lesson> updateDefaultLanguage(@PathVariable String listId, @RequestBody Map<String, String> payload) {
        String defaultLanguage = payload.get("defaultLanguage");
        listService.updateLanguage(listId, defaultLanguage);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
    }
}
